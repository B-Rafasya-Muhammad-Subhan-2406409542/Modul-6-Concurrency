...
Pada tahapan ini, saya mengimplementasikan *multithreading* untuk menyelesaikan masalah *blocking* pada *single-thread* menggunakan pola `ThreadPool`. Daripada membuat *thread* baru secara tak terbatas untuk setiap *request* yang bisa menghabiskan memori dan memicu serangan DoS, server kini menyiapkan batasan 4 *worker thread* di awal. `ThreadPool` utama bertugas menerima *request* yang masuk dan mengirimkannya ke *worker* yang sedang menganggur menggunakan *channel* komunikasi (`mpsc`). Agar *receiver* dari *channel* tersebut bisa dibagi secara aman ke banyak *worker*, saya menggunakan kombinasi *smart pointer* `Arc` (Atomic Reference Counted) dan `Mutex` untuk mengunci data. Hasilnya sangat memuaskan: ketika rute lambat `/sleep` diakses, ia hanya akan memblokir 1 *worker* saja, sementara 3 *worker* lainnya tetap terbebas untuk melayani *request* normal secara instan dan asinkronus tanpa harus menunggu antrean panjang.

## Commit Bonus Reflection notes

Pada tahap bonus ini, saya melakukan perbaikan fungsi pada pembuatan instans `ThreadPool`. Berdasarkan konvensi bahasa Rust, sebuah fungsi bernama `new` secara implisit diharapkan tidak akan pernah mengalami kegagalan (*panic*). Sebelumnya, penggunaan `assert!(size > 0)` di dalam fungsi `new` melanggar konvensi ini karena berpotensi membuat program *crash* secara paksa jika diberikan ukuran kolam *thread* bernilai nol. Oleh karena itu, saya mengganti nama fungsinya menjadi `build` yang mengindikasikan proses inisialisasi ini bisa saja gagal. Selain itu, fungsi `build` kini mengembalikan tipe data `Result<ThreadPool, PoolCreationError>` dibandingkan langsung mengembalikan instans. Pendekatan ini membuat program jauh lebih aman dan robust karena menggeser tanggung jawab penanganan *error* ke fungsi *caller* (pemanggil), memungkinkan program utama untuk menangani kegagalan inisialisasi secara elegan tanpa harus mengalami *crash* mendadak.